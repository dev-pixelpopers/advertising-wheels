                ...
                <Reveal className="mt-14 grid grid-cols-1 gap-5 md:grid-cols-2" y={48} stagger={0.14}>
                    {BENEFITS.map((b) => (
                        ...
                        >
                            ...
                            <h3 className="mt-7 font-tommy-bold text-[23px] leading-tight tracking-tight text-[#1A1917] md:text-[27px]">
                                {b.k}
                            </h3>
                            <p className="mt-4 font-tommy-regular text-[15px] leading-[1.7] text-[#5A554C] md:text-[16px]">
                                {b.d}
                            </p>
                        </div>
                    ))}
        ...
        <section className="w-full border-y border-black/10 bg-[#E7E0CE] py-24 text-[#1A1917] transition-colors duration-300 md:py-32">
            <div className="mx-auto max-w-[1000px] px-6 md:px-12">
                ...
                <div ref={rootRef} className="relative mt-16 pl-[70px]">
                    ...
                    <div className="absolute left-[26px] top-2 bottom-2 w-px bg-black/12">
                        <div ref={lineRef} className="h-full w-full origin-top bg-[#C8992B]" />
                    </div>

                    <div className="flex flex-col gap-14">
                        {STEPS.map((s, i) => (
                            <div key={s.step} className="vstep relative">
                                <span className="absolute left-[-70px] flex h-[54px] w-[54px] items-center justify-center rounded-full border-2 border-[#C8992B] bg-[#E7E0CE] font-tommy-bold text-[20px] text-[#C8992B]">
                                    {i + 1}
                                </span>
                                <h3 className="font-tommy-bold text-[26px] tracking-tight md:text-[30px]">{s.step}</h3>
                                <p className="mt-3 max-w-[560px] font-tommy-regular text-[16px] leading-[1.72] text-[#5A554C]">
                                    {s.body}
                                </p>
                            </div>
                        ))}
        ...
        <section className="w-full bg-[#EEE8D9] py-24 transition-colors duration-300 md:py-32">
            <div className="mx-auto grid max-w-[1200px] grid-cols-1 gap-12 px-6 md:px-12 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20">
                <Reveal className="self-start" y={30}>
                    ...
                    <p className="mt-5 max-w-[420px] font-tommy-regular text-[16px] leading-[1.7] text-[#5A554C]">
                ...
                <Reveal className="grid grid-cols-1 gap-3 sm:grid-cols-2" y={30} stagger={0.09}>
                    {REQUIREMENTS.map((r) => (
                        <div
                            key={r}
                            className="flex items-start gap-3.5 rounded-2xl border border-black/10 bg-white/40 px-5 py-4"
                        >
                            ...
                            <p className="font-tommy-regular text-[15px] leading-[1.5] text-[#3A3730]">{r}</p>
                        </div>
                    ))}
                </Reveal>
            </div>
            ...
            <Reveal className="mx-auto grid max-w-[1200px] grid-cols-2 gap-y-12 px-6 md:px-12 lg:grid-cols-4" y={30} stagger={0.14}>
                {NETWORK_STATS.map((s, i) => (
                    <div key={i} className="border-l border-black/10 pl-6">
                        <p className="font-tommy-bold text-[clamp(40px,5.5vw,72px)] leading-none tracking-tight text-[#1A1917]">
                            {s.el}
                        </p>
                        <p className="mt-3 font-tommy-regular text-[13px] uppercase tracking-[2px] text-[#6F6A60]">
                            {s.label}
                        </p>
                    </div>
                ))}
