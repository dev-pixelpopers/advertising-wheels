...
/**
 * MarketsCoverage V2 ΓÇö the roll call.
 *
 * No map, no trucks. The section pins and plays against one scrubbed timeline:
 *
 *   ACT 1  Intro ΓÇö "Where We Roll", the national totals, the invitation to open
 *          the coverage list.
 *   ACT 2  Roll call ΓÇö the covered STATES scroll vertically past a fixed
 *          reading line, set oversized in the wordmark weight.
 *   ACT 3  Detail ΓÇö clicking a state that holds several markets opens its
 ...
 *          chevron marks the states that hold more. Clicking a city ΓÇö or a
 *          single-market state, which has nothing to open ΓÇö brings up the
 *          full-screen record.
 *
 * TWO INVARIANTS HOLD THIS TOGETHER ΓÇö break either and the highlight drifts off
 * the reading line:
 *
 *   1. EVERY rendered row is the same height, city rows included. The active
 *      index is then just `-y / rowHeight` ΓÇö O(1), no layout reads.
 ...
 * scrollΓåÆy mapping. That is handled explicitly rather than avoided: the
 ...
 * that re-seat and you get the classic desync ΓÇö reading line on row 27, list
 * showing row 0.
 */
 ...
 * project's only custom breakpoint ΓÇö see `--breakpoint-3xl` in globals.css).
 ...
 *
 * Tuned so the list travels roughly 1:1 with the scroll ΓÇö at 61 rows (31 states
 ...
 *
 *   0        ΓåÆ HOLD      intro sits still, fully readable
 *   HOLD     ΓåÆ SWAP_END  intro out and roll in, over the SAME window
 *   SWAP_END ΓåÆ INTRO_END roll settled at full opacity, nothing moving
 *   INTRO_END ΓåÆ 1        the list travels
 *
 * The crossfade endpoints deliberately coincide. When they didn't, there was a
 * trough around 0.05ΓÇô0.08 where the intro had dropped to ~40% and the roll was
 * only ~30% in ΓÇö so the heading looked "gone" and the names looked washed out,
...
/**
 * Flattens the groups into the scrolling row set ΓÇö ONLY the rows that are
 ...
 * changes the scrollΓåÆy mapping. That mapping is re-seated explicitly
 * afterwards (see `pendingSeatRef` and the effect that consumes it) so the
 * state you clicked stays exactly where you clicked it ΓÇö without that
 * re-seat you get the classic desync: reading line on row 27, list showing
 * row 0.
 *
 * Single-market states get NO child row ΓÇö a state whose only market is the
 ...
 *
 * The share bar is drawn against THIS, not against 100% ΓÇö no market can
 ...
 *
 *   backdrop dims and blurs ΓåÆ the card unfolds from a horizontal slit
 *   (clip-path) ΓåÆ the photo settles out of an overscaled push-in while a
 *   light sweeps across it ΓåÆ the copy stacks up in a stagger ΓåÆ the figures
...
function MarketPopup({
     ...
     * single record ΓÇö it looks like a statistic and carries no information.
     * The figures below were picked for the opposite reason: they actually
     * separate one market from another.
     */
    /** Where this market sits in the 50 ΓÇö 100% for #1, ~2% for #50. */
    const rankPct = ((TOTALS.count - market.rank + 1) / TOTALS.count) * 100;
    /** This market's share of the national 18+ reach: ~12% (NYC) ΓåÆ ~0.5%. */
    const shareOfReach = (market.adults / TOTALS.adults) * 100;
    /** The same share as a bar width ΓÇö see MAX_SHARE_OF_REACH. */
    const sharePct = (shareOfReach / MAX_SHARE_OF_REACH) * 100;
    /** Delivery against audience size ΓÇö a ~4x spread across the footprint. */
    ...
    useGSAP(
        () => {
            ...
            /* Figures are authored at their FINAL value in the markup (so the
               ...
               Queried off the ref, NOT via a bare selector ΓÇö useGSAP scopes
            ...
            tl.fromTo(backdropRef.current, { autoAlpha: 0 }, { autoAlpha: 1, duration: 0.35 }, 0)
                ...
                /* Photo settles out of a push-in, so the banner has depth
                   ...
                   1.06, NOT 1 ΓÇö that overscan is the headroom the pointer
            ...
            /* Pointer parallax ΓÇö the photo drifts against the card, so the
     ...
     * opening ΓÇö no curtain, no logo, the card never leaves. Mount is skipped
    ...
    return (
        <div ref={overlayRef} className="absolute inset-0 z-[200] flex items-center justify-center p-4 md:p-8">
            ...
            {/* Transition curtain ΓÇö blooms over everything, carries the mark,
            ...
            >
                {/* Close sits on the CARD, not the photo ΓÇö on desktop the photo
                ...
                {/* Photo left, record right. The photo column has no height of
                    its own past mobile ΓÇö it stretches to whatever the content
                ...
                <div className="grid md:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
                    <div className="relative h-[210px] overflow-hidden md:h-auto">
                        ...
                        {/* Specular sweep ΓÇö see the timeline above. */}
                        <span
                            ref={sweepRef}
                            aria-hidden="true"
                        ...
                        {/* Title sits ON the photo ΓÇö the record then opens on the
                    ...
                    {/* The floor stops a market with no audience note from
                        collapsing the card into a letterbox ΓÇö it drives the
                    ...
                    <div className="relative lg:max-h-[86vh] min-h-0 overflow-y-auto p-3 md:p-6 md:min-h-[min(430px,74vh)] lg:p-9">
                        {/* The rank, set as a watermark ΓÇö gives the data column
                        ...
                        <div className="relative">
                            ...
                            {/* Two measures that actually move market to market ΓÇö
                            ...
                            <div data-pop-item className="mt-3 md:mt-5 lg:mt-7 flex flex-col gap-y-2 md:gap-y-3 lg:gap-y-4">
                                ...
                                <div>
                                    ...
                                    {/* Shares top out around 12%, so the track is
                                        scaled to that rather than to 100% ΓÇö at true
                            ...
                            {/* Somewhere to go next ΓÇö see `related` above. Set as
...
export default function MarketsCoverageV2() {
     ...
     * on the row it was over ΓÇö so without this the disc would sit behind the
     ...
     * close over the count ΓÇö it has to look it up on each tick instead.
     */
    const rowCountRef = useRef(0);
    /** Last count the refresh effect acted on ΓÇö lets it skip the mount pass. */
     ...
     * scrollΓåÆy mapping. Set this on the click, consume it in the effect that
     * watches `rows`, which restores the SAME on-screen position afterwards.
     *
     * Position, not index ΓÇö that distinction is the whole point. Seating the
     ...
     * OLD row set ΓÇö the two together survive the rows shifting underneath.
    ...
    /**
     * The ONE state whose city dropdown is open, if any ΓÇö accordion, not a
    ...
    // Changes on region filter AND on expand/collapse ΓÇö the effect below
    ...
    /**
     * The state the reading line is currently inside ΓÇö a state row, or any of
     ...
     * stable ΓÇö it's a dependency of the effect below, and a new identity every
    ...
    /**
     * Inverse of `targetY` ΓÇö scrolls to whatever position puts the list at
    ...
    /**
     * Built ONCE, on mount ΓÇö note the empty dependency list.
     *
     ...
     * `invalidate()` + `refresh()` (see the effect below) ΓÇö the trigger, the pin
    ...
    useGSAP(
        () => {
            ...
            /* Act 1 ΓÇö intro out and roll in, across ONE shared window.
             *
             ...
             * the intro ΓÇö and the heading never came back. `fromTo` re-records
            ...
            /* Act 2 ΓÇö the roll call. Function value + invalidateOnRefresh so the
    ...
    /**
     * The row count changed ΓÇö from an expand/collapse, or a region filter.
     * Either way the scrollΓåÆy mapping is now stale, so re-evaluate it and
    ...
    useEffect(() => {
        ...
        // No target ΓÇö a region change. That one DOES reset to the top, since
     ...
     * first ΓÇö the effect above then re-anchors the scroll to it once the new
     * rows have rendered.
     */
     ...
     * under the pointer ΓÇö worst for the top rows, which could only ever be
     * dragged downwards.
     */
     ...
     * names what the row beneath it does ΓÇö Open / Close / View.
     *
    ...
    useGSAP(
        () => {
            ...
            if (!zone || !disc) return;
            // Coarse pointers have no hover to follow ΓÇö leave them the native cursor.
            ...
            const onMove = (e: PointerEvent) => {
                ...
                qx(e.clientX - r.left);
                qy(e.clientY - r.top);
                // Re-read every move so the label follows OpenΓåÆClose the
     ...
     * measured against a document that is still growing ΓÇö so by the time you
    ...
    /**
     * The falloff is encoded ENTIRELY in solid colour ΓÇö no `opacity-*`.
     *
            ...
            <div ref={pinRef} className="relative h-screen w-full overflow-hidden">
                ...
                >
                    ...
                    <p className="mt-9 font-tommy-regular text-[10px] uppercase tracking-[3px] text-[#8A857C] dark:text-[#9A968E]">
                        Scroll to explore the coverage areas Γåô
                    </p>
                </div>
                ...
                <div ref={rollRef} className="absolute inset-0 z-10 flex flex-col pt-[3%]">
                    {/* Persistent section header.
                        ...
                        needs to say what it is ΓÇö losing the title entirely left the
                    ...
                    {/* `min-h-0` so the list column can actually shrink inside the
                        ...
                        No side panel anymore ΓÇö a market's record opens in the
                    ...
                    <div className="mx-auto grid min-h-0 w-full max-w-[1440px] flex-1 grid-cols-1 px-4 md:px-8 lg:px-12">
                        ...
                        <div ref={listZoneRef} className="relative h-full overflow-hidden min-h-[30vh] lg:min-h-0">
                            {/* Custom cursor ΓÇö a disc that trails the pointer and names
                            ...
                            {/* Names dissolve at the edges rather than being hard-clipped.
                                ...
                                `to-transparent` ΓÇö that resolves to rgba(0,0,0,0), so the
                            ...
                            {/* Offset by HALF A ROW, not half the track's own height ΓÇö
                                ...
                                <ul ref={listRef} className="will-change-transform">
                                    {rows.map((row, i) => {
                                        ...
                                        if (row.kind === 'state') {
                                            ...
                                            // state ΓÇö either on it, or on one of its cities
                                            // below it.
                                            ...
                                            return (
                                                <li key={row.key} className="flex h-[32px] items-center md:h-[40px] lg:h-[50px] xl:h-[60px] 3xl:h-[84px]">
                                                    ...
                                                    >
                                                        ...
                                                        {/* Multi-market states always carry this chevron ΓÇö
                                                        ...
                                                        {/* Single-market states have no dropdown ΓÇö this is
                                                        ...
                                                        {!many && (
                                                            ...
                                                            >
                                                                ...
                                                                <span className="text-[#C8992B] dark:text-[#FCD119]">
                                                                    {isPopupOpen ? 'ΓùÅ open' : 'Explore ΓåÆ'}
                                                                </span>
                                                            </span>
                                                        )}
                                                        {many && (
                                        ...
                                        // City row ΓÇö only ever rendered while its state is
                    ...
                    {/* Full-screen popup ΓÇö replaces the old side panel entirely.
